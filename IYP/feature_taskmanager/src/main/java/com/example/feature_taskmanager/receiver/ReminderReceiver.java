...
//	}
//}
package com.example.feature_taskmanager.receiver;
...
import com.example.feature_taskmanager.R;
import com.example.feature_taskmanager.TaskDetailActivity;
...
public class ReminderReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (taskId == -1 || taskTitle == null) {
			Log.e("ReminderReceiver", "Missing taskId or taskTitle in intent!");
			return;
		}

		Log.d("ReminderReceiver", "Alarm received → taskId: " + taskId + ", title: " + taskTitle);

		Toast.makeText(context, "Reminder: " + taskTitle, Toast.LENGTH_SHORT).show();

		// Open TaskDetailActivity when tapped
		Intent openIntent = new Intent(context, TaskDetailActivity.class);
		openIntent.putExtra("taskId", taskId);
